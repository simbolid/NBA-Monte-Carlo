...
# cannot use lambda in parallel process
def dict_func():
    return defaultdict(int)


def playoff_worker(num_simulations, east_teams, west_teams, ratings):
    stage_counts = defaultdict(dict_func)
...
def merge_results(dicts):
    ...
    for d in dicts:
        for team, stages in d.items():
            for stage, count in stages.items():
                merged[team][stage] += count
    return merged
...
def get_playoff_odds(num_simulations, num_workers, east_teams, west_teams, ratings):
    ...
    with mp.Pool(num_workers) as pool:
        results = pool.starmap(
            playoff_worker, 
            [(chunk, east_teams, west_teams, ratings) for chunk in chunks]
        )
    
    # combine results
    combined_counts = merge_results(results)
    
    # calculate probabilities
    playoff_odds = defaultdict(dict)
    for team, stages in combined_counts.items():
        for stage, count in stages.items():
